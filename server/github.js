...
function setupGithub({ server }) {
  ...
  const CLIENT_ID = dev ? process.env.GITHUB_TEST_CLIENT_ID : process.env.GITHUB_LIVE_CLIENT_ID;
  const API_KEY = dev ? process.env.GITHUB_TEST_KEY : process.env.GITHUB_LIVE_KEY;
...
function getRepos({ accessToken }) {
  ...
  return github.repos.listForUser({ username: 'ryanyogan', per_page: 100 });
}
